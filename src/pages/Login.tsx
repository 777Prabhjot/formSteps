...
const Login = () => {
  ...
  const handleSubmit = () => {
    setErrorMsg('')
    setSuccessMsg('')
    onLogin({email: values.email, password: values.password}).then((res) =>{
        if(res.status === 200){
    ...
    <div className="bg-yellow-400 h-screen overflow-hidden flex items-center justify-center">
      ...
      <div className="bg-white lg:w-5/12 md:6/12 w-10/12 shadow-3xl">
        ...
        <div className="p-12 md:p-24">
          ...
          <button disabled={!values.email || !values.password} className="bg-gradient-to-b from-gray-700 to-gray-900 font-medium p-2 md:p-4 text-white uppercase w-full" onClick={handleSubmit}>
            Login
          </button>
        </div>
      </div>
